...
public class OperationServiceTest {
	...
	/*
	 * test depot somme positive
	 */
	@Test
	public void depositInAccountTest() throws IOException {
	...
	/*
	 * test depot somme negative
	 */
	@Test
	public void depositNegativeInAccountTest() throws IOException {
		acc.setBalance(1000L);
		ope.setAmount(-1000L);
		ope.setOperationType(OperationType.DEPOSIT);
		Assertions.assertThatThrownBy(() -> operationService.deposit(ope, acc)).isInstanceOf(IOException.class);
	}

	/*
	 * test retrait somme inferieur au solde
	 */
	@Test
	public void withDrowlFromAccountTest() throws IOException {
		acc.setBalance(1000L);
		ope.setAmount(500L);
	...
	/*
	 * test retrait tout le solde
	 */
	@Test
	public void withDrowlAllFromAccountTest() throws IOException {
		acc.setBalance(1000L);
		ope.setOperationType(OperationType.WITHDRAWL);
	...
	/*
	 * test retrait tout le solde vide
	 */
	@Test
	public void withDrowlAllEmptyFromAccountTest() throws IOException {
		acc.setBalance(0L);
		ope.setOperationType(OperationType.WITHDRAWL);
		Assertions.assertThatThrownBy(() -> operationService.withDrowlAll(ope, acc)).isInstanceOf(IOException.class).hasMessage("Balance is empty");
	
	}
	@Test
	public void withDrowlNegFromAccountTest() throws IOException {
			acc.setBalance(1000L);
			ope.setAmount(-1000L);
			Assertions.assertThatThrownBy(() -> operationService.withDrowl(ope, acc))
	        .isInstanceOf(IOException.class).hasMessage("Amount Negative");
			}
	
	/*
	 * test retrait somme superieur au solde
	 */
	@Test
	public void withDrowlSupBalanceFromAccountTest() throws IOException {
		acc.setBalance(1000L);
		ope.setAmount(1500L);
		ope.setOperationType(OperationType.WITHDRAWL);
		Assertions.assertThatThrownBy(() -> operationService.withDrowl(ope, acc)).isInstanceOf(IOException.class).hasMessage("Amount bigger than Balance");
	}
	
	
	@Test
	public void getMyHistoryTest() throws IOException {
		...
		List<Operation> resOpe = operations.stream().filter(opera->opera.getAccount() == myAccount).collect(Collectors.toList());
